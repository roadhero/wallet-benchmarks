...
| `fixtures/bip39_wordlist.txt` | BIP39 English wordlist (2048 words, public-domain, vendored) | Yes | `src/result/redaction.rs::tests` — used to build the BIP39-detection regex |
...
The fake-secret test scaffold (directive 7) is constructed *programmatically* inside `src/result/redaction.rs::tests` and never serialized to disk — no committed file ever contains a "realistic" BIP39 phrase or view key. The fake values use the standard BIP39 test mnemonic ("abandon abandon … about") plus syntactically-valid-but-disposable Tari testnet view-only addresses generated for this purpose. Documenting this convention in `tests/README.md` is part of `swe-test`'s job.
...
**Test seeds are deterministic + obviously-fake.** The single test mnemonic used across all unit tests is the BIP39 standard test phrase: `"abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"` (12 words, valid BIP39 with zero entropy). Documented in `tests/README.md` as "the all-zeros test vector — never use anywhere a real funded wallet might exist". The redaction test (directive 7) also uses this same string as one of the values in the "REDACTION_DENYLIST patterns must match" assertions — i.e. the redaction test deliberately seeds the in-memory profile state with `"abandon abandon ... about"` and then asserts the serialized JSON does NOT contain it.

**Realistic-looking secrets for the redaction test** are constructed programmatically:

- BIP39 seeds: the all-zeros phrase above + a second phrase generated by sampling deterministic indices into the BIP39 wordlist (seed = 42) — 24-word version, demonstrates the test works on both 12 and 24-word phrases.
- View keys: a syntactically valid Tari testnet address with the view-key prefix, computed once at test-fixture-build time from the all-zeros seed and pinned as a `const` in the test module.
