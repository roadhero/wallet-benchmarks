...
mod tests {
    ...
    async fn s3_outputs_match_expected_when_counts_agree() {
        ...
        let outcome = run(&owner.ctx(), &mut fake, 128, h_birth)
            .await
    ...
    async fn s3_outputs_mismatch_records_false_without_bailing() {
        ...
        let owner = TestCtxOwner::new();
        let outcome = run(&owner.ctx(), &mut fake, 127, 365)
            .await
    ...
    #[tokio::test]
    async fn s3_populates_peaks_from_sampler() {
        let mut fake = FakeMode::new();
        fake.canned_scan = Some(canned_scan(128));
        let factory = FakeSamplerFactory::new(vec![16384], vec![]);
        let mut owner = TestCtxOwner::new();
        owner.config.sampler_interval_ms = 5;
        let outcome = run(&owner.ctx_with_sampler(&factory), &mut fake, 128, 365)
            .await
            .expect("S3 runs");
        assert_eq!(outcome.peak_rss_bytes, Some(16384));
