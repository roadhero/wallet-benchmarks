...
| `s0_change_confirm_timeout_secs` | `600` (10 min) | Bounds S0's post-send settle gate, the pre-send entry gates at S0 and S1 (the wallet must hold at least 1 confirmed spendable input before sending; see section 7.11), AND S1's per-send settle between chained sends. `0` skips the three gates; the per-send settle then falls back to its shared 600 s default (it is never skipped — settling between chained sends is what makes S1's serial contract measurable). |
